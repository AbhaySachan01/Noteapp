...
function Home() {
  ...
  const renderCards = (items) => {
    return items.map((item, key) => (
      <div
        key={key}
        className="shadow-lg rounded-lg overflow-hidden transform transition-all hover:scale-105 p-2 m-4"
        style={{
      ...
      <div className="flex flex-col items-center pt-24 px-4 md:px-6 lg:px-8">
        ...
        {filteredItems.length === 0 && searchQuery !== "" ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4 w-full max-w-screen-lg">
            {searchQuery === "" ? renderCards(sortArray) : renderCards(filteredItems)}
          </div>
        )}
      </div>
    </section>
